...
class Dispatcher {
    ...
    dispatch(req: any, res: any) {
        ...
        let resource = fullPath.pathname ?? ''; // Aggiungi un fallback per evitare null
    
        let params = fullPath.query;
    
        console.log(`${this.prompt}${method}: ${resource} params: ${JSON.stringify(params)}`);  // if (!resource?.startsWith("/api/")) { //per togliere il ? usa un fallback
